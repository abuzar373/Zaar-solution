...
/**
 * Interactive chrome for the admin area. The session is already verified on
 * the server by the layout, so this component only handles UI state:
 * sidebar toggle, theme and logout.
 */
    ...
    <ToastProvider>
      <div className="flex min-h-screen">
        ...
        >
          ...
          <nav className="space-y-1 overflow-y-auto p-4 pb-40">
            {NAV.map((item) => {
              const active =
                item.href === "/admin"
                  ? pathname === "/admin"
          ...
          <div className="absolute inset-x-0 bottom-0 border-t border-slate-200/60 dark:border-white/10 bg-white/80 dark:bg-[#0a0f1e]/90 p-4">
            ...
            <div className="flex gap-2">
              <button
                onClick={toggleTheme}
                className="flex-1 rounded-lg border border-slate-300/60 dark:border-white/10 px-3 py-2 text-xs font-medium hover:bg-slate-500/10"
              >
                {dark ? "☀️ Light" : "🌙 Dark"}
              </button>
              <button
                onClick={logout}
                disabled={loggingOut}
                className="flex-1 rounded-lg border border-rose-500/30 bg-rose-500/10 px-3 py-2 text-xs font-semibold text-rose-500 hover:bg-rose-500/20 disabled:opacity-60"
              >
                {loggingOut ? "…" : "Logout"}
              </button>
            </div>
        ...
        <div className="flex-1 lg:pl-64">
          <header className="sticky top-0 z-20 flex h-16 items-center justify-between border-b border-slate-200/60 dark:border-white/10 bg-white/70 dark:bg-[#070b16]/80 glass px-4 sm:px-6">
            <button
              ...
              className="grid h-10 w-10 place-items-center rounded-lg border border-slate-300/60 dark:border-white/10 lg:hidden"
            >
              ☰
            </button>
            ...
            <Link
              href="/"
              target="_blank"
              className="rounded-lg border border-slate-300/60 dark:border-white/10 px-4 py-2 text-sm font-medium hover:bg-slate-500/10"
            >
              View Website ↗
            </Link>
          </header>
